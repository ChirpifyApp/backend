import { Body, Controller, Delete, Get, MaxFileSizeValidator, Param, ParseFilePipe, Post, UploadedFile, UseGuards, UseInterceptors } from '@nestjs/common';
...
export class PostsController {
    ...
    async createPost(@UserDecorator() user: User, @Body() createPostDto: CreatePostDto, @UploadedFile(new ParseFilePipe({
        validators: [
    ...
    @UseGuards(JwtAuthGuard)
    @Get('/recent/:page')
    async getRecentPosts(@Param('page') page: string) {
        let take = 10;
        let skip = (parseInt(page) - 1) * take;
        return await this.postsService.getRecentPosts(skip, take, parseInt(page));
    }
